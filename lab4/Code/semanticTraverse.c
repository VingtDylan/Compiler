...
void Dec(TreeNode *root,Type type,char *pname,int index){
  FieldList field=VarDec(root->child[0],type,pname,index);
  if(field->type->kind==ARRAY&&index==1){
    ...
    decIR->kind=DEC_CODE;
    decIR->u.decOp.op=op;
    decIR->u.decOp.size=getSize(field->type,0);
    insertCode(decIR);
  }
  if(root->child_num==3){
    //VarDec ASSIGNOP Exp(place)
    Operand place=(Operand)malloc(sizeof(Operand_));
    Type type;
    if(strcmp(root->child[2]->child[0]->name,"INT")==0){
      type=Exp(root->child[2],NULL);
      place->kind=CONSTANT_OP;
      sprintf(place->u.value,"%s",root->child[2]->child[0]->value);
    }else{
      place->kind=TEMP_VAR_OP;
      type=Exp(root->child[2],place);
    }
    if(place->kind!=VARIABLE_OP||strcpy(place->u.value,field->name)!=0){
      ...
      strcpy(leftOp->u.value,field->name);
      InterCode addrIR=(InterCode)malloc(sizeof(InterCode_));
      addrIR->kind=ASSIGN_CODE;
      addrIR->u.assignOp.left=leftOp;
      addrIR->u.assignOp.right=place;
      insertCode(addrIR);
    }
  }
  ...
  if(indexField!=NULL){
    if(strstr(indexField->parent,pname)!=NULL){
      printf(CYAN"Error type 3: Redefined variable \"%s\".\n"NONE,field->name);
    }
    else{
...
void traverseDefList(TreeNode *root,char* pname){
  ...
  while(DefList!=NULL){
    ...
    while(DecList->child_num==3){
      /*Dec COMMA DecList*/
      Dec(DecList->child[0],symbolType,pname,1);
